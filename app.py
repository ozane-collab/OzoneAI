...
app = Flask(__name__)
# የትኛውንም አይነት የ Cross-Origin የጥሪ ችግር ለመከላከል
...
def home():
    return "OzoneAI Multilingual Engine is Live!", 200
...
def chat():
    try:
        ...
        # 🎨 1. PHOTO MODE ማስተካከያ (ምስሉ በፍሮንትኤንድ ላይ ፍንትው ብሎ እንዲታይ)
        if chat_mode == "photo":
            ...
            # HTML ስክሪፕትህ 'image' ወይም 'url' ወይም 'reply' ውስጥ ያለውን ሊንክ ቢፈልግ እንኳ እንዳያጣው ሁሉንም እንሞላለን
            return jsonify({
                "reply": f"🎨 Generated image for: '{user_message}'\n\n<img src='{generated_image_url}' style='max-width:100%; border-radius:8px; margin-top:10px;' />",
                "image": generated_image_url,
                "url": generated_image_url
            })

        # 🤖 2. UNRESTRICTED MODE ማስተካከያ (እንቅፋት የሌለው ነጻ ሞዴል)
        elif chat_mode == "unrestricted":
            if not openrouter_client or not OPENROUTER_API_KEY:
                return jsonify({"reply": "⚠️ OpenRouter API Key is missing on Render. Please double check!"}), 401
            ...
            system_instruction = (
                ...
                "If they use Amharic script or Latin-Amharic (Pinglish like 'Endet neh', 'Selam', 'Sra yelehm'), reply in Amharic using Ethiopic script."
            )
            ...
            try:
                completion = openrouter_client.chat.completions.create(
                    ...
                    model="google/gemma-2-9b-it:free", # የተረጋገጠ ነጻና ክፍት ሞዴል
                    messages=[
        ...
        # 🤖 3. CHAT MODE ማስተካከያ (እንግሊዝኛ + አማርኛ ሚዛናዊ አደረጋጋጭ)
        else:
            if not groq_client:
            ...
            system_instruction = (
                ...
                "1. If the user writes in English (e.g., 'Hi', 'Hello', 'How are you', 'What can you do'), you MUST reply in English.\n"
                "2. If the user writes in Amharic script (e.g., 'እንዴት ነህ', 'ሰላም'), you MUST reply in Amharic using Ethiopic script.\n"
                "3. If the user writes in Latin-Amharic / Pinglish (e.g., 'Endet neh', 'Selam', 'Srah mndnew', 'Alen'), detect that this is Amharic language typed with English letters, and reply in Amharic using Ethiopic script.\n"
